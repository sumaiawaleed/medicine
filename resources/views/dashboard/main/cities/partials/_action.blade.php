<div class="btn-group">
    ...
    <div class="dropdown-menu" role="menu">
        @if (auth()->user()->hasPermission('cities-update'))
            <a onclick="return edit_row('{{ route(env('DASH_URL').'.cities.edit',$id) }}')" class="dropdown-item"
               href="#">@lang('site.edit')</a>
        @endif

        @if (auth()->user()->hasPermission('cities-delete'))
            <form onsubmit="return delete_process('{{ route(env('DASH_URL').'.cities.remove',$id) }}')" id="delete-{{ $id }}"
                  class="delete-form"
                  action="{{ route(env('DASH_URL').'.cities.remove',$id) }}"
