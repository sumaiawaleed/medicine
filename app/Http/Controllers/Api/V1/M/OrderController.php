...
class OrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($user->type == 4) {
            $data['orders'] = Order::where('m_id', $user->id)
                ->orderByDesc('id')
                ->paginate(20);
    ...
    public function update($id, Request $request)
    {
        ...
        $user = $request->user('api');
        $lang = $request->header('lang');
        if ($user->type == 4 and $order->m_id == $user->id) {
            try {
                $order_data = [
                    'client_id' => $request->client_id,
                    'total' => ($request->total) ? $request->total : 0,
    ...
    public function show($id,Request $request){
        ...
        if ($user->type == 4 and $order->m_id == $user->id) {
            ...
            $data['order'] = $order;
            $order_items  = OrderItem::with(['product','unit'])->where('order_id',$id)->get();
            $items = array();
            foreach ($order_items as $index=>$item){
                ...
                $items[$index]['unit_name'] = $item->unit ? $item->unit->getTranslateName($lang) : '';
            }
            $data['items'] = $items;
    ...
    public function edit_item(Request $request){
        ...
        $user = $request->user('api');
        $lang = $request->header('lang');
        if ($user->type == 4 and $order->m_id == $user->id and $item->order_id = $request->order_id) {
            $item_data = [
                'product_id' => $request->product_id,
                'order_id' => $request->order_id,
                'quantity' => $request->quantity,
